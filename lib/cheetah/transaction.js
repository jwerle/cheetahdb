...
Transaction.prototype.exec = function(mode) {
	var stores = {}, trans = this.connection.db.transaction(this.collections, mode || this.mode)
	this.collections.map(function(collection){ stores[collection] = trans.objectStore(collection); });
	return stores;
};
